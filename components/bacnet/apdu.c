    ...
    uint16_t * service_request_len)
{
    ...
    service_data->segmented_message = (apdu[0] & BAC_BIT(3)) ? true : false;
    service_data->more_follows = (apdu[0] & BAC_BIT(2)) ? true : false;
    service_data->segmented_response_accepted =
        (apdu[0] & BAC_BIT(1)) ? true : false;
        ...
        switch (apdu[0] & 0xF0) {
            ...
            case PDU_TYPE_COMPLEX_ACK:
                service_ack_data.segmented_message =
                    (apdu[0] & BAC_BIT(3)) ? true : false;
                service_ack_data.more_follows =
                    (apdu[0] & BAC_BIT(2)) ? true : false;
